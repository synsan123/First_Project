...
# 데이터 및 이미지를 불러오기 위한 매개변수 설정
detection_model_path = 'haarcascade_files/haarcascade_frontalface_default.xml'  # 얼굴 감지 모델 경로
emotion_model_path = 'models/_mini_XCEPTION.102-0.66.hdf5'  # 감정 분류 모델 경로
...
while True:
    ...
    for (fX, fY, fW, fH) in faces:
        ...
        # 배경과 전경 이미지 결합
        output_frame = cv2.addWeighted(bg_image, 0.7, bg_removed_result, 0.3, 0)
        
        # 프레임에 감정 라벨 표시
        cv2.putText(output_frame, label, (fX, fY - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 255, 255), 2)
        cv2.rectangle(output_frame, (fX, fY), (fX + fW, fY + fH), (255, 255, 255), 2)
